...
export default function ListarAreas({ setAlert, windowHeight, isMaximized }) {
  ...
  const {
    ...
    maxPage,
    fetchElements
  } = useFetch({ setAlert, windowHeight, isMaximized, obtener: 'areas' })

  const { submitData } = useEdit({ setAlert, obtener: 'area' })
  ...
  const {
    ...
    setShowModal,
    handleDeactivate
  } = useDeactivate({ setAlert, obtener: 'area', fetchElements })

  const {
    formRef,
    handleSubmit
  } = useCreate({ setAlert, obtener: 'area', fetchElements })
